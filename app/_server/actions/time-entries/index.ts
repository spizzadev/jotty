...
async function getCategoryRegistry(
  username: string,
...
async function saveCategoryRegistry(
  username: string,
...
async function getOrCreateCategoryId(
  ...
  const oldFilePath = path.join(
    ...
    `${CAT_PREFIX}${oldSlug}.json`,
  );
  const newFilePath = path.join(
    process.cwd(),
    TIME_ENTRIES_DIR(username),
    `${CAT_PREFIX}${newId}.json`,
  );
  try {
...
async function getCategoryFilePath(
  username: string,
  categoryName: string,
): Promise<string> {
...
export const getAllTimeEntries = async (
  ...
  try {
    ...
    const entryFiles = files.filter(
      (f) =>
        f.endsWith(".json") && f !== BILLING_FILE && f !== CAT_REGISTRY_FILE,
    );
...
export const getTimeEntrySummary = async (
  ...
  success: boolean;
  data?: { taskIds: string[]; categories: string[] };
  error?: string;
}> => {
  try {
    const user = await resolveUser(usernameOverride);
    ...
    const registry = await getCategoryRegistry(user.username);
    ...
    for (const file of entryFiles) {
      ...
      const hasCompleted = (data as ProjectTimeEntry[]).some(
        (e) => e.durationMin !== undefined,
      );
      if (!hasCompleted) continue;

      if (file.startsWith(CAT_PREFIX)) {
        const catId = file.slice(CAT_PREFIX.length, -5);
        const catName = registry[catId];
    ...
    return { success: true, data: { taskIds, categories } };
  } catch (error) {
